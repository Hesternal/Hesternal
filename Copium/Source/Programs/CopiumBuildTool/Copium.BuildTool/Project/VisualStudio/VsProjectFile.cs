...
internal sealed class VsCppProjectFile : VsProjectFile
{
    ...
    public VsCppProjectFile(DirectoryItem projectDir, CppTarget cppTarget)
        : base(projectDir, cppTarget.TargetName, VsFileExtension.Vcxproj)
    {
        m_projectGuid = ProjectFileExist ? _GetGuidFromProjectFile() : VisualStudioGuid.Generate();

        if (cppTarget.eTargetType == BuildRules.TargetType.Engine)
        {
