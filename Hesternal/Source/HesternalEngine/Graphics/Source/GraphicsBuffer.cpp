...
namespace Hesternal
{
    ...
    {
        HS_ASSERT_MSG(m_graphicsBufferDesc.ElementCount > 0, "Trying to create GraphicsBuffer with ElementCount < 1");
        HS_ASSERT_MSG(m_graphicsBufferDesc.ElementSize > 0, "Trying to create GraphicsBuffer with ElementSize < 1");

        // NOTE(v.matushkin): No way to check this anymore?
        //if (m_graphicsBufferDesc.Usage == GraphicsBufferUsage::Index)
        //{
        //    HS_ASSERT_MSG(m_graphicsBufferDesc.ElementSize == 2 || m_graphicsBufferDesc.ElementSize == 4,
        //                   "For Index GraphicsBuffer ElementSize should be either 2 or 4");
        //}
        //else
        if (m_graphicsBufferDesc.Usage == GraphicsBufferUsage::Constant)
        {
            HS_ASSERT_MSG(m_graphicsBufferDesc.ElementSize % 4 == 0,
        ...
        HS_ASSERT_MSG(initialData.size() <= m_sizeInBytes, "initialData is bigger than GraphicsBuffer can hold");
